...
def Gurobi_STC(dataset_name, store_file_name, run_id, n=-1, tol = 1e-5):
    ...
    if gurobi_model.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and gurobi_model.SolCount > 0:
        ...
        if misclassified > 0:
            with open(f"Stats/STC/{dataset_name}_gurobi_log_tol.csv", "a") as f:
                f.write(f"Tol:{tol}\nMisclassified: {misclassified}\n")
            Gurobi_STC(dataset_name, store_file_name, run_id, n=n, tol=tol+5e-6)

        print(f"Total misclassified samples: {misclassified}")
        with open(f"Stats/STC/{dataset_name}_gurobi_log.csv", "a") as f:
            f.write(f"-----\nRun ID: {run_id}\n\n")
...
def Gurobi_CmC_Correct(dataset_name, store_file_name, run_id, n=-1, tol = 1e-5, misclassification_count=1):
    X_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_inputs_train.pt").numpy()
    labels_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_labels_train.pt").numpy()
    pred_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_preds_train.pt").numpy()
    X_full_size = X_full.shape[0]
    ...
    for s in range(n_samples):
        ...
        if labels_gt[s] == label_max:
            ...
            for k in range(l2_size):
                if k != label_max:
                    gurobi_model.addConstr((violations[k] == 1) >> (Z2[label_max] <= Z2[k] - tol), name=f"violation_1_CmC_{s}_{k}")
                    gurobi_model.addConstr((violations[k] == 0) >> (Z2[label_max] >= Z2[k] + tol), name=f"violation_0_CmC_{s}_{k}")
                else:
            ...
            for k in range(l2_size):
                if k != label_max:
                    gurobi_model.addConstr(Z2[label_max] >= Z2[k] + tol, name=f"violation_0_CmC_{s}_{k}")
            gurobi_model.addConstr(misclassified_flags[s] == 0, name=f"misclassified_flag_{s}")
    ...
    if gurobi_model.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and gurobi_model.SolCount > 0:
        ...
        if misclassified != misclassification_count:
            with open(f"Stats/CmC/{dataset_name}_gurobi_log_tol.csv", "a") as f:
                f.write(f"Tol:{tol}\nMisclassified: {misclassified}\n")
            Gurobi_CmC_Correct(dataset_name, store_file_name, run_id, n=n, tol=tol+5e-6, misclassification_count=misclassified_count)

        print(f"Total misclassified samples: {misclassified}")
        with open(f"Stats/CmC/{dataset_name}_gurobi_log.csv", "a") as f:
            f.write(f"-----\nRun ID: {run_id}\n\n")
            f.write("-------Weight/Bias Offsets-------\n")
            f.write(f"W2 offsets: {np.sum(np.abs(W2_off))}\n")
            f.write(f"b2 offsets: {np.sum(np.abs(b2_off))}\n")
...
def Gurobi_CmC_Any(dataset_name, store_file_name, run_id, n=-1, tol = 1e-5, misclassification_count=1):
    X_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_inputs_train.pt").numpy()
    labels_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_labels_train.pt").numpy()
    pred_full = torch.load(f"checkpoints_inputs/{dataset_name}/fc_preds_train.pt").numpy()
    X_full_size = X_full.shape[0]
    ...
    if gurobi_model.status in [GRB.TIME_LIMIT, GRB.OPTIMAL] and gurobi_model.SolCount > 0:
        ...
        if misclassified != misclassification_count:
            with open(f"Stats/CmC/{dataset_name}_gurobi_log_tol.csv", "a") as f:
                f.write(f"Tol:{tol}\nMisclassified: {misclassified}\n")
            Gurobi_CmC_Any(dataset_name, store_file_name, run_id, n=n, tol=tol+5e-6, misclassification_count=misclassified_count)
